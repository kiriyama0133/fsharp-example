...
type Manager() =
    ...
    member _.TryGetNativeHandle(windowId: WindowId) =
        tryGetWindow windowId
        |> Option.bind (fun window -> window.TryGetNativeHandle())

    member _.TryGetWin32Hwnd(windowId: WindowId) =
        tryGetWindow windowId
        |> Option.bind Platform.TryGetWin32Hwnd
